            	...
            	<div class="container">
            		<div class="row">
						...
						<div class="address col-12 order-md-first col-md-4 col-lg-5">
							<div class="branding"><img src="<?php bloginfo('stylesheet_directory'); ?>/images/company_logo.png" alt="<?php echo(get_field('company', 'options')); ?>"/></div>
							<div class="address-container">
								<!--<?php if (isset($company)) { ?><p><?php echo($company); ?></p><?php } ?>-->
								<?php if (isset($address)) { ?>
									<p><?php echo($address['street_address']); ?></p>
									<?php if($address['street_address2']) { ?><p><?php echo($address['street_address2']); ?></p><?php } ?>
									<p><?php echo($address['city']['city']); ?>, <?php echo($address['city']['state']); ?> <?php echo($address['city']['zip']); ?></p>
								<?php } ?>
								<?php if (isset($contact)) { ?>
									<p class="phone"><a href="tel:<?php echo($contact['phone']); ?>"><?php echo($contact['phone']); ?></a></p>
									<?php if (isset($contact['fax']) && $contact['fax'] != '') { ?><p class="phone"><?php echo($contact['fax']); ?></p><?php } ?>
									<?php if (isset($contact['email']) && $contact['email'] != '') { ?><p class="email"><a href="mailto:<?php echo($contact['email']); ?>"><?php echo($contact['email']); ?></a></p><?php } ?>
								<?php } ?>
								<?php  
